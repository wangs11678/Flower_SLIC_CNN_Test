 ...
 * Output: -
 */
void Slic::display_rect_box(IplImage *image, vector<IplImage*> &imageRect, CvScalar colour) {
	for (int i = 0; i < (int) centers.size(); i++) {
		IplImage *imgcopy = cvCloneImage(image);	
        cvRectangle(imgcopy, cvPoint(centers[i][3]-40, centers[i][4]-40), cvPoint(centers[i][3]+40, centers[i][4]+40), colour, 1, 4, 0);
        //IplImage *img = cvCloneImage(imgcopy);	
        imageRect.push_back(imgcopy);
    }
}

/*
 * Extract the rectangle ROI.
 *
 * Input : The image to display upon (IplImage*).
 * Output: -
 */
void Slic::extract_rect_ROI(IplImage *image, vector<IplImage*> &seg) {
	for (int i = 0; i < (int) centers.size(); i++) {
		cvSetImageROI(image, cvRect(centers[i][3]-40, centers[i][4]-40, 80, 80)); 
